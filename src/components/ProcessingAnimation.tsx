...
export default function ProcessingAnimation() {
  ...
  useEffect(() => {
    ...
    const init = (forceW?: number | Event, forceH?: number) => {
      ...
      width = explicitW || containerRef.current?.clientWidth || window.innerWidth || 800;
      height = explicitH || containerRef.current?.clientHeight || window.innerHeight || 600;
    ...
    const stepFrame = (targetCtx: CanvasRenderingContext2D) => {
      time += 0.002;
    ...
    exportRef.current = async () => {
      ...
      // Ensure 4K resolution at 3:4 aspect ratio => 2160x2880
      width = 2160;
      height = 2880;
      time = 0;
      ...
      try {
        ...
        a.href = url;
        a.download = 'digital_animation_loop_4k.mp4';
        a.click();
    ...
    <div className={`fixed inset-0 overflow-hidden bg-neutral-950 selection:bg-transparent flex flex-col items-center justify-center ${isTD ? 'p-0 m-0 bg-black' : 'p-4 sm:p-8'}`}>
      <div 
        ...
        style={isTD ? { width: '100%', height: '100%' } : {
          width: '100%',
          maxWidth: 'calc((100vh - 8rem) * 0.75)',
          aspectRatio: '3 / 4'
        }}
        className={`relative bg-black shadow-2xl overflow-hidden shrink-0 ${isTD ? 'w-full h-full' : 'rounded-lg sm:rounded-2xl border border-neutral-800'}`}
      >
          ...
          >
            {isExporting ? (
              <span className="flex items-center gap-2">
                ...
                Exporting 4K MP4 Loop ({Math.round(exportProgress * 100)}%)
              </span>
            ) : (
